# Setze den Pfad zum ffmpeg-Executable und zum Zielordner
$ffmpegPath = "ffmpeg.exe"
$sourceFolder = "done"
$destinationFolder = "02"
$numOfCores = (Get-CimInstance -ClassName Win32_Processor).NumberOfLogicalProcessors
...
foreach ($file in $files) {
    ...
    $process = Start-Process -FilePath $ffmpegPath -ArgumentList "-y -hide_banner -hwaccel auto -i ""$($sourceFile)"" -threads $numOfCores -c:v libvvenc -preset fast -c:a copy ""$($destinationFile)""" -WindowStyle Maximize -PassThru 
    #-WhatIf
